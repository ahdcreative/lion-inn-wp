...
// Debugging functions 
require_once(plugin_dir_path(__FILE__).'/le-debug.php');
...
class LESQLManager {
    ...
    private function createTable($table) {
        ...
        $sql = file_get_contents( WP_PLUGIN_DIR  . "/lion-events/assets/sql/create_" . $table . "_table.sql" );

        // Set table name
        $sql = str_replace("tableplaceholder", $this->wpdb->prefix . "le_" . $table, $sql);

        // If sql contains a foreign key - add prefix
        if (strpos($sql, 'FOREIGN KEY') !== false) {
            $sql = str_replace("prefixplaceholder", $this->wpdb->prefix . "le", $sql);
        }
    ...
    private function deleteTable($table) {
        $sql = file_get_contents( WP_PLUGIN_DIR  . "/lion-events/assets/sql/delete_table.sql" );
    ...
    public function insert($table, $params) {
        $table = $this->wpdb->prefix . "le_" . $table;
    ...
    public function get($table, $where = array()) {
        ...
        if($where) {
            $sql = file_get_contents( WP_PLUGIN_DIR  . "/lion-events/assets/sql/select_where_" . $table . ".sql" );
            ...
            $sql = file_get_contents( WP_PLUGIN_DIR  . "/lion-events/assets/sql/select_all_" . $table . ".sql" );
        }
